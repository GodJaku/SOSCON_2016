...
namespace {
    ...
    uString *NSString2uString(NSString *str)
    {
        ...
        uString *result = ::uAllocString((int) length);
...
struct HttpRequest::Private
{
    ...
    void Completed(NSData *data, NSHTTPURLResponse *response, NSError *error)
    {
        ...
        if (data && data.length)
        {
            ...
            {
                case 0:       // String
                    this_->responseContent_ = ::uNewStringUtf8(
                        (const char *) data.bytes, (int) data.length);
                    break;

                case 1:    // ByteArray
                    this_->responseContent_ = ::uNewArray(
                        ::app::Uno::Byte__typeof(), (int) data.length, data.bytes);
                    break;
...
uString *HttpRequest::GetResponseHeaders() const
{
    ...
    uString *result = uAllocString((int) (resultLength - 1));
...
{
    return ::uAs< ::uString *>(
        (uObject *&)responseContent_, ::app::Uno::String__typeof());
}

uArray *HttpRequest::GetResponseContentByteArray() const
{
    return ::uAs< ::uArray *>(
        (uObject *&)responseContent_, ::uGetArrayType(::app::Uno::Byte__typeof()));
}
